...
export default async function LeapsPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 selection:bg-emerald-500 selection:text-white font-sans">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-white tracking-tight">LEAPS Strategies</h1>
                <p className="text-zinc-400 mt-1">Identify long-term options for sustainable portfolio growth.</p>
              </div>
              {/* Strategy Indicator */}
              <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-emerald-900/20 border border-emerald-500/20 rounded-full">
                <span className="relative flex h-2 w-2">
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                </span>
                <span className="text-xs font-medium text-emerald-400">Long-Term Mode</span>
              </div>
            </div>

            {/* The Tool Card */}
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
               {/* Background Glow Effect (Green for Leaps) */}
               <div className="absolute top-0 right-0 -mt-20 -mr-20 w-80 h-80 bg-emerald-600/5 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <p className="text-xs text-zinc-500 flex items-start gap-2 bg-zinc-900/30 p-4 rounded-lg border border-zinc-800/50">
              <svg className="w-4 h-4 text-zinc-400 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <span>
                <strong>Strategy Note:</strong> LEAPS (Long-Term Equity Anticipation Securities) are designed for time horizons of 1+ years. 
                Focus on fundamentals and macro trends rather than intraday noise.
              </span>
            </p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-zinc-900 to-zinc-900 border border-zinc-800 rounded-2xl p-6">
              <h3 className="text-lg font-bold text-white mb-2">Want Quick Action?</h3>
              <p className="text-sm text-zinc-400 mb-4">
                Switch to standard Signals for high-frequency intraday setups.
              </p>
              <Link 
                href="/signals" 
              ...
              >
                Go to Intraday Signals &rarr;
              </Link>
            </div>

            {/* Educational / Status */}
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
              <h3 className="text-sm font-bold text-zinc-300 uppercase tracking-wider mb-4">LEAPS Guidelines</h3>
              <ul className="space-y-3 text-sm text-zinc-400">
                <li className="flex gap-3">
                  <span className="text-emerald-500 font-bold">✓</span>
                  <span>Look for deep ITM (In-The-Money) calls (Delta &gt; 0.80).</span>
                </li>
                <li className="flex gap-3">
                  <span className="text-emerald-500 font-bold">✓</span>
                  <span>Target expiration dates at least 12 months out.</span>
                </li>
                <li className="flex gap-3">
                  <span className="text-emerald-500 font-bold">✓</span>
                  <span>Avoid high IV (Implied Volatility) environments.</span>
                </li>
              </ul>
            </div>
